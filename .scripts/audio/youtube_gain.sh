...
while [[ "$flag" -eq 0 && "$i" -lt 20 ]]; do
    ...
    n_non_browser="$(pactl list sink-inputs | awk '/Corked:|application.name / {print $0};' | grep --after-context 1 "Corked: no" | grep "application.name" | grep -v "LibreWolf" | grep -v "Firefox" | grep -v "Tor Browser" | wc -l)"
